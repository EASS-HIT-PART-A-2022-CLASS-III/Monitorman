...
st.selectbox(
    "Interval (Minutes)", [1, 2, 5, 10, 30, 60], key="add_minute_interval", index=2
)
st.number_input(
    "Expected Status", step=1, min_value=0, value=200, key="add_expected_status"
)
st.text_input("Expected Result Regex", key="add_expected_result_regex")
st.text_input("Expected Headers Regex", key="add_expected_headers_regex")
st.number_input(
    "Expected Max Duration(ms)",
    step=1,
    min_value=0,
    key="add_expected_max_duration_ms",
    value=10000,
)
    ...
    for key in as_dict:
        if key.startswith("add_"):
            if "expected_" in key:
                ...
                send_obj["checks"][key.removeprefix("add_")] = as_dict[key]
            else:
                send_obj[key.removeprefix("add_")] = as_dict[key]

    res = requests.post(f'{os.getenv("BACKEND_URL")}/monitors/v1', json=send_obj)
